...
class GameWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.size_label.setText(
            f"{self.col_slider.value()} X {self.col_slider.value()}")
    ...
    def loadBOT(self, modelBase: str, m, n, ver=None):
        ...
        if modelBase == "Resnet":
            if ver:
                ...
                args_Res['load_model_name'] = None
            bot = ResnetBOT(input_size_m=m, input_size_n=n,
                            game=self.game, args=args_Res)
    ...
    def OnClickStartButton(self):
        ...
        self.P1_score_label.setText(
            f'Player1 scores: {self.game.state.p1_p2_scores[0]}')
        self.P2_score_label.setText(
            f'Player2 scores: {self.game.state.p1_p2_scores[1]}')
        self.winner_label.setText("")
        ...
        if self.p1 == "人類":
            ...
            self.botOppo = self.loadBOT(
                modelBase=self.p1, m=self.game.state.m, n=self.game.state.n, ver=ver)
        ...
        if self.p2 == "人類":
            ...
            ver = self.p2_ver_input_box.text()
            self.botOppo = self.loadBOT(
                modelBase=self.p2, m=self.game.state.m, n=self.game.state.n, ver=ver)
    ...
    def game_loop(self):
        if isGameOver(self.game.state.board):
            ...
            winner = GetWinner(self.game.state.board,
                               self.game.state.p1_p2_scores)
            if winner == -1:
        ...
        if self.p1 == -1 and self.p2 != 1:   # p1人類, p2機器
            if self.game.state.current_player == -1:
                ...
                r, c = self.p2.get_move(
                    self.game.state.board, self.game.state.current_player)[0]
                self.game.state.current_player, score = make_move(
                    self.game.state.board, r, c, self.game.state.current_player)
                if self.game.state.current_player == -1:
            ...
            self.mouse_events_enabled = False
            if self.game.state.current_player == -1:
                (r, c), his = self.p1.get_move(
                    self.game.state.board, self.game.state.current_player)
                self.game.history.append(his)
            else:
                (r, c), his = self.p2.get_move(
                    self.game.state.board, self.game.state.current_player)
                self.game.history.append(his)
            self.game.state.current_player, score = make_move(
                self.game.state.board, r, c, self.game.state.current_player)
        ...
        for i in range(self.game.state.board_rows):
            for j in range(self.game.state.board_cols):
                ...
                if self.game.state.board[i][j] == -1:
                    ...
                    if i % 2 == 0:  # 偶數列，水平線
                        painter.drawLine(
                            x - self.gap + self.Dots_radius - 1, y, x + self.gap - self.Dots_radius, y)
                    else:
                        painter.drawLine(
                            x, y - self.gap + self.Dots_radius, x, y + self.gap - self.Dots_radius)
                    ...
                    painter.drawPixmap(
                        x - self.gap // 2, y - self.gap // 2, self.gap, self.gap, self.box1_pixmap)
                elif self.game.state.board[i][j] == 9:
                    painter.drawPixmap(
                        x - self.gap // 2, y - self.gap // 2, self.gap, self.gap, self.box2_pixmap)
                if self.game.state.board[i][j] == 5:
                    painter.setBrush(QColor('#000000'))  # 設定填充顏色為黑色
                    painter.drawEllipse(x - self.Dots_radius // 2, y - self.Dots_radius //
                                        2, self.Dots_radius, self.Dots_radius)  # 繪製實心圓
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:  # 確保是左鍵點擊
            ...
            row = y // self.gap
            col = x // self.gap
            print(find_chain(self.game.state.board, row, col, self.game.state.board_rows,
                  self.game.state.board_cols, self.game.state.current_player))
            self.game.state.current_player, score = make_move(
                self.game.state.board, row, col, self.game.state.current_player)
            if self.game.state.current_player == -1:
                self.game.state.p1_p2_scores[0] += score
            elif self.game.state.current_player == 1:
                self.game.state.p1_p2_scores[1] += score
