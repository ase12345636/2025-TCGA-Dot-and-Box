...
def dualMCTS(n_game, bot1, bot1_name, exp=2):
    """
    讓兩個 Bot 進行多場對戰，記錄勝負結果
    """
    print(f"{bot1_name} VS AlphaGo".center(76))
    os.makedirs("game_record", exist_ok=True)  # 確保資料夾存在
    file_path = f'game_record/{bot1_name} VS AlphaGo.txt'

    bot1_win, bot2_win = 0, 0

    with open(file_path, "a") as f:
        f.write(f"{bot1_name} VS AlphaGo".center(76) + "\n")

        # bot2 = MCTSPlayer(10000, game_state, 1, exp)
        bot2 = AlphaGoMCTSPlayer(400, game, 1, exp)
        for i in range(1, n_game + 1):
            print(f"Game {i}")
            ...
            if result == -1:
                ...
                print('Draw!')
            # 記錄結果
            record_result(f, i, bot1_name, "AlphaGo", bot1_win, bot2_win)
            print(f"{bot1_name} win: {bot1_win}")
            print(f"AlphaGo win: {bot2_win}")
            print("-" * 76)
...
def dualMCTS_AB(n_game):
    """
    讓兩個 Bot 進行多場對戰，記錄勝負結果
    """
    print(f"C_AB VS AlphaGo".center(76))
    os.makedirs("game_record", exist_ok=True)  # 確保資料夾存在
    file_path = f'game_record/C_AB VS AlphaGo.txt'

    bot1_win, bot2_win = 0, 0

    with open(file_path, "a") as f:
        f.write(f"C_AB VS AlphaGo".center(76) + "\n")
        bot1 = C_AB_player(-1, game.state, 5)
        # bot2 = MCTSPlayer(10000, game_state, 1, exp)
        bot2 = AlphaGoMCTSPlayer(500, game, 1, 2)
        for i in range(1, n_game + 1):
            print(f"Game {i}")
            ...
            if result == -1:
                ...
                print('Draw!')
            # 記錄結果
            record_result(f, i, "C_AB", "AlphaGo", bot1_win, bot2_win)
            print(f"C_AB win: {bot1_win}")
            print(f"AlphaGo win: {bot2_win}")
            print("-" * 76)
