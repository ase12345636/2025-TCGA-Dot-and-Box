...
int GetWinner(const vector<vector<int>>& board, const vector<int>& scores) {
    if (!isGameOver(board)) return 999;
...
int alphabeta(STATE state, int depth, int max_depth, int alpha, int beta, int symbol, bool maximizing) {
    ...
    if (maximizing) {
        ...
        for (const auto& move : validmoves) {
            ...
            if (new_state.current_player == -1) new_state.p1_p2_scores[0] += score;
            if (new_state.current_player == 1) new_state.p1_p2_scores[1] += score;
            // 檢查有無換人，確認是否繼續最大化
            int eval = 0;
